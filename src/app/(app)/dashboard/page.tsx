...
export default async function DashboardPage() {
  ...
  // Parallel queries
  const [totalRecords, byStatus, byType, expenseAgg, recentRecords] = await Promise.all([
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-baseline justify-between">
        ...
        <p className="text-slate-400 text-xs">{formatDate(now)}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-4 flex items-center gap-3">
        ...
        <div>
          <p className="text-xs text-slate-500">ค่าใช้จ่ายเดือนนี้</p>
          <p className="text-3xl font-black text-slate-900 leading-tight">
            ฿{monthlyExpenses.toLocaleString("th-TH", { minimumFractionDigits: 2 })}
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-2">
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-semibold text-slate-800 text-sm">จำแนกตามประเภท</h2>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-semibold text-slate-800 text-sm">จำแนกตามสถานะ</h2>
          </div>
          <div className="space-y-2">
            {byStatus.length === 0 && (
              <p className="text-slate-400 text-sm">ยังไม่มีข้อมูล</p>
      ...
      {/* Recent records */}
      <div className="bg-white rounded-xl border border-slate-100 p-4">
        <h2 className="font-semibold text-slate-800 text-sm mb-3">บันทึกล่าสุด</h2>
        {recentRecords.length === 0 && (
          <p className="text-slate-400 text-sm">ยังไม่มีบันทึก</p>
        )}
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-400">
                  {RECORD_TYPE_LABELS[r.type as RecordType]} · {formatDate(r.updatedAt)}
                </p>
              </div>
              <span
