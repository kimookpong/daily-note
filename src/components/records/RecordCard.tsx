...
import { Cloud, CloudOff, AlertCircle, MapPin, Tag, Banknote } from "lucide-react"
...
export function RecordCard({ record }: { record: LocalRecord }) {
  const statusColor = RECORD_STATUS_COLORS[record.status as RecordStatus] ?? "bg-slate-100 text-slate-600"
        ...
        <div className="ml-auto flex items-center gap-2 text-xs text-slate-400">
          {record.location && (
            <span className="flex items-center gap-0.5">
              ...
              <span className="hidden sm:inline truncate max-w-[80px]">{record.location}</span>
            </span>
          )}
          {record.tags.length > 0 && (
            <span className="flex items-center gap-0.5">
              <Tag className="w-3 h-3" />
              {record.tags[0]}{record.tags.length > 1 && ` +${record.tags.length - 1}`}
            </span>
          )}
          {totalExpenses > 0 && (
